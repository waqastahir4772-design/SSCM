...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_partner'])) {
    ...
    $capacity = escape($_POST['manufacturing_capacity'] ?? '');
    
    $sql = "INSERT INTO scm_partners (name, code, type, contact_person, phone, email, manufacturing_capacity, status) 
            VALUES ('$name', '$code', '$type', '$contact', '$phone', '$email', '$capacity', 'Active')";
    
    if ($conn->query($sql)) {
        redirect('manufacturers.php', "$type added successfully!");
    } else {
            ...
            <tbody>
              <?php if ($partners && $partners->num_rows > 0): ?>
                ...
                <tr>
                  ...
                  <td><?php echo h($p['manufacturing_capacity']); ?></td>
      ...
      <form action="" method="post">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input type="text" name="manufacturing_capacity" class="form-control" placeholder="e.g. 1000 units/day">
          </div>
        </div>
